...
class three_zone_torque(planktos.swarm):
    ...
    def ODEs(self, t, states):
        ...
        sum= np.zeros(N_attract) #initialize the sum of the attraction angle terms
        for n in range(N_attract):
            
            #dist_n= self.__calc_dist(positions[n, :], positions) #distance from particle n to all other particles
            ...
            for j in range(N_attract):
                if j != n:  # don't include self
                    ...
                    #fraction = (positions[j, :] - positions[n, :]) / (dist_n[j] + 0.0000001)
                    fraction = (diffs_n[j]) / (dist_n[j] + 0.0000001)
...
df.to_csv('examples/MK-torque/attraction/speed_history_attraction-corrected10.csv')
...
swrm.plot_all(movie_filename=f'examples/MK-torque/attraction/torque-attraction-corrected10.mp4', fps=100, fluid='quiver', plot_heading=True) # realtime #4 #min v0, max v0, r_r, r_o, r_a, dt, constant xflow, constant yflow, iter, frames