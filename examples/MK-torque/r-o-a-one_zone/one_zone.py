...
class three_zone_torque(planktos.Swarm):
    ...
    def ODEs(self, t, states):
        ...
        for n in range(N_attract):
            ...
            sum_attract[n] = np.arctan2(np.sum(y_comp_n_attract)/len(y_comp_n_attract), np.sum(x_comp_n_attract)/len(x_comp_n_attract))
        ...
        for n in range(N_repel):
            ...
            # Average direction angle
            sum_repel[n] = np.arctan2(np.sum(y_comp_n_repel)/len(y_comp_n_repel), np.sum(x_comp_n_repel)/len(x_comp_n_repel))
...
swrm.plot_all(movie_filename=f'examples/MK-torque/r-o-a-one_zone/torque-one_zone-corrected10.mp4', fps=100, fluid='quiver', plot_heading=True) # realtime #4 #min v0, max v0, r_r, r_o, r_a, dt, constant xflow, constant yflow, iter, frames