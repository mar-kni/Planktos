...
class three_zone_torque(planktos.swarm):
    ...
    def ODEs(self, t, states):
        ...
        sum= np.zeros(N_align) #initialize the sum of the attraction angle terms
        for n in range(N_align):
            ...
            for j in range(N_align):
                if j != n:  # don't include self
                    ...
                    fraction = np.sin(self.get_prop('angle')[j]- self.get_prop('angle')[n])
                    fractions_n.append(fraction)

            # Average direction angle
            sum[n] = np.sum(fractions_n)
            
        d_angle= (self.get_prop('K')/(N_align)) * sum
...
df.to_csv('examples/MK-torque/alignment/speed_history_alignment-corrected10.csv')
...
swrm.plot_all(movie_filename=f'examples/MK-torque/alignment/torque-alignment-corrected10.mp4', fps=100, fluid='quiver', plot_heading=True) # realtime #4 #min v0, max v0, r_r, r_o, r_a, dt, constant xflow, constant yflow, iter, frames