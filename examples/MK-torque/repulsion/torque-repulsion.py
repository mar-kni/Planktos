...
class three_zone_torque(planktos.swarm):
    ...
    def ODEs(self, t, states):
        ...
        sum= np.zeros(N_repel) #initialize the sum of the attraction angle terms
        for n in range(N_repel):
            ...
            for j in range(N_repel):
                if j != n:  # don't include self
    
                    # Unit vector from n to j
                    fraction = (diffs_n[j]) / (dist_n[j] + 0.0000001)**2

                    # Store x and y components directly
                    x_comp_n.append(fraction[0])
                    y_comp_n.append(fraction[1])


            # Average direction angle
            sum[n] = np.arctan2(np.sum(y_comp_n)/len(y_comp_n), np.sum(x_comp_n)/len(x_comp_n))
            
        d_angle= -self.get_prop('K')* np.sin(sum-self.get_prop('angle'))
...
#corner initial conditions
# IC_pos[0,0] = 0.05 #set x initial position for particle 0
# IC_pos[0,1] = 0.05 #set y initial position for particle 0
# IC_pos[1,0] = 0.05 #0.15  #set x initial position for particle 1
# IC_pos[1,1] = 0.1 #set y initial position for particle 1
# IC_pos[2,0] = 0.1
# IC_pos[2,1] = 0.05
# IC_pos[3,0] = 0.05
# IC_pos[3,1] = 0.9
...
df.to_csv('examples/MK-torque/repulsion/speed_history_repulsion-corrected10.csv')
...
swrm.plot_all(movie_filename=f'examples/MK-torque/repulsion/torque-repulsion-corrected10.mp4', fps=100, fluid='quiver', plot_heading=True) # realtime #4 #min v0, max v0, r_r, r_o, r_a, dt, constant xflow, constant yflow, iter, frames